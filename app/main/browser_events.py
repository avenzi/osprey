...
def upload(data):
    ...
    if not content:
        error("Uploaded file '{}' has no content - aborted".format(name))
        return
...
def database_info(group_name):
    ...
    if database.live:
        ...
        speed = database.playback_speed

    for stream in group_info.keys():
        group_info[stream]['speed'] = speed
