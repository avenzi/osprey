(function (global, factory) {
  ...
  function searchByteArray(buffer, searchElements, fromIndex) {
    // find first instance of a sequence of bytes in a ByteArray
    ...
    for (var i = index, j = 0; j < searchElements.length && i < buffer.length; i++, j++) {
      if (buffer[i] !== searchElements[j]) {
        return searchByteArray(buffer, searchElements, index + 1);
      }
    }
  ...
  */
  ...
  var AACParser = /*#__PURE__*/function () {
    _createClass(AACParser, null, [{
      ...
      value: function extractAAC(buffer) {
        // assumes buffer begins with a complete ADTS header
        var i = 0,
            length = buffer.byteLength,
            result = [],
            header_index,
        ...
        if (!AACParser.isAACPattern(buffer)) {
          error('Invalid ADTS header format: ', _toConsumableArray(new Uint8Array(buffer.slice(0, 7))).map(function (b) {
            return b.toString(2).padStart(2, '0');
          }).join(''));
          header_index = searchByteArray(buffer, [0xff, 0xf1]); // search for syncwords of frame

          if (header_index == -1) {
            // no header found
            console.log('no header found');
            return [result, new Uint8Array()]; // return nothing, throw out entire buffer
          } else {
            // header found
            buffer = buffer.slice(header_index); // slice until header
        ...
        while (i < length) {
          ...
          if (frameLength > buffer.byteLength) {
            // next frame wasn't completely sent
            break;
          }
        ...
        return [result, buffer]; // return complete frames and leftover incomplete frame
      }
    }, {
      key: "samplingRateMap",
      get: function get() {
  ...
  var JMuxer = /*#__PURE__*/function (_Event) {
    ...
    function JMuxer(options) {
      ...
      _this.kfCounter = 0;
      _this.pendingUnits = {};
      _this.remainingVideoData = new Uint8Array();
      _this.remainingAudioData = new Uint8Array();
      /* events callback */
    ...
    _createClass(JMuxer, [{
      ...
      value: function feed(data) {
        ...
        if (data.video) {
          data.video = appendByteArray(this.remainingVideoData, data.video);
          ...
          left_video = _H264Parser$extractNA2[1];
          ...
          this.remainingVideoData = left_video || new Uint8Array();
        }

        if (data.audio) {
          data.audio = appendByteArray(this.remainingAudioData, data.audio);

          var _AACParser$extractAAC = AACParser.extractAAC(data.audio);

          var _AACParser$extractAAC2 = _slicedToArray(_AACParser$extractAAC, 2);

          slices = _AACParser$extractAAC2[0];
          left_audio = _AACParser$extractAAC2[1];
          ...
          this.remainingAudioData = left_audio || new Uint8Array();
        }
