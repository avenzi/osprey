...
class Base:
    ...
    def debug(self, msg, level=1):
        ...
        if Base.debug_level >= level:
            self.display("[{}][{}]: {}".format(self.get_time(), self, msg))
...
class Streamer(WorkerNode):
    ...
    def connect_socket(self):
        ...
        while not self.exit:
            try:
                ...
                self.debug("Failed to connect to server socketIO: {}".format(self, e), 1)
                pass
            time.sleep(5)
    ...
    def update(self):
        ...
        try:
            ...
            self.debug("Unable to update database - database still loading.", 3)
        except DatabaseTimeoutError as e:
            self.debug("Unable to update database - connection times out.", 3)
        except Exception as e:
    ...
    def json(self, dic):
        ...
        self.debug("Received JSON: {}".format(self.name, dic))
        pass
...
class Analyzer(Streamer):
    ...
    def get_target(self, stream_id=None):
        ...
        try:
            ...
            self.debug("Failed to get target info from database: {}".format(self, e), 3)
            return

        if not info_list:
            self.debug("Database returned no info for the targets")
            return
    ...
    def _start(self):
        ...
        if any(streams):  # any of the target streams is present
            ...
            self.debug("Not started - did not find any target streams.".format(self))
